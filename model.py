...
class skipthought(object):
    ...
    def __init__(self, mode, path, folds, embedding_size, hidden_size, hidden_layers, batch_size, keep_prob_dropout, L2, learning_rate, val_size, bidirectional, num_epochs = 100):
    ...
    def summary_stats(self, lengths, labels, name):
        ...
        table.index = pd.Index(['[0, 100)', '[100, 500)', '[500, 1000]', '[1000, 1500)', '[1500, 2000)','[2000, inf]'], name="Bin")
    ...
    def run(self):
        ...
        if self.is_train:
            ...
            if self.folds == 1:
                ...
                self.val_statistics, self.val_frame = self.summary_stats(self.val_lengths, self.val_labels, 'validation')
        ...
        if self.is_train == True:
            with tf.Session() as session:
                ...
                train_summary_writer = tf.summary.FileWriter(str(self.path + 'tensorboard/train_summary'), session.graph)
                ...
                try:
                    ...
                    for epoch in range(self.num_epochs):
                        ...
                        for i in range(n // self.batch_size):
                            ...
                            _, batch_loss, batch_accuracy, batch_summary = session.run([opt_op, loss, accuracy, merged], feed_dict=train_dict)
                            ...
                            if i%10 == 0 and i > 0:
                                ...
                                string_out = '\rEnd of batch ' + str(i) + '    Train loss:   ' +  str(total_loss / (i*self.batch_size)) + '    Accuracy:   '+  str(batch_accuracy)
                        ...
                        train_summary, train_loss, train_accuracy = session.run([merged, loss, accuracy], feed_dict=train_dict)
            ...
            with tf.Session() as session:
                ...
                self.val_pred, self.val_accuracy, self.val_probs = session.run([pred, accuracy, probs], feed_dict=val_dict)

                _ = self.summary_stats(self.val_lengths, self.val_labels, 'val')

                print('\nConfusion matrix (all sequence lengths):')
                val_confusion_1 = self.confusion(gold = self.val_labels, prediction = self.val_pred, lengths = self.val_original_lengths, min_length = 0, max_length = np.inf)
                print(val_confusion_1)

                print('\nConfusion matrix (sequence length < 2000):')
                val_confusion_2 = self.confusion(gold = self.val_labels, prediction = self.val_pred, lengths = self.val_original_lengths, min_length = 0, max_length = 2000)
                print(val_confusion_2)

                print('\nConfusion matrix (sequence length > 2000):')
                val_confusion_3 = self.confusion(gold = self.val_labels, prediction = self.val_pred, lengths = self.val_original_lengths, min_length = 2000, max_length = np.inf)
                print(val_confusion_3)

                print('\n Val accuracy:', self.val_accuracy)
                print('\n Val accuracy when length <2000:', np.sum((self.val_pred == self.val_labels) * (self.val_original_lengths <= 2000))/np.sum(self.val_original_lengths <= 2000))
                print('\n Val accuracy when length >2000:', np.sum((self.val_pred == self.val_labels) * (self.val_original_lengths > 2000))/np.sum(self.val_original_lengths > 2000))
